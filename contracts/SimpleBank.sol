...
contract SimpleBank {
    ...
    event LogFailedPayment(address accountAddress, uint amount);
    ...
    function withdraw(uint withdrawAmount) public payable returns (uint) {
      ...
      if (!address(msg.sender).send(withdrawAmount)) {
        emit LogFailedPayment(msg.sender, msg.value);
      }
      else { balances[msg.sender]-=withdrawAmount;
